...
exports.refresh = async (req, res) => {
  try {
    ...
    jwt.verify(refreshToken, process.env.JWT_SECRET);
    let newAccessToken = jwt.sign({ email: user.email }, process.env.JWT_SECRET, { expiresIn: '15m' });
    res.cookie('accessToken', newAccessToken, { httpOnly: true, maxAge: 15 * 60 * 1000, secure: true, sameSite: 'none' });
    res.status(200).json({ message: 'Токен оновлено' });
  } catch (error) {
    res.status(401).json({ message: 'Токен прострочений або недійсний' });
  }
...
exports.createProducts = async (req, res) => {
    try {
        const { title, price, sizes, description, category, type, date } = req.body;
        if (!req.file) return res.status(400).json({ message: "Фото товару є обов'язковим!" });
        ...
        const newProduct = new Product({
            title, price: Number(price), sizes: parsedSizes, description, category, type, imageUrl, date
        });
...
exports.updateProduct = async (req, res) => {
    try {
        const { id, title, price, sizes, description, category, type } = req.body;
        ...
        let updatedData = {
            title, price: Number(price), sizes: parsedSizes, description, category, type
        };
...
exports.getProducts = async (req, res) => {
  try {
    ...
    if (category && category !== 'allCategories') query.category = category;
    if (type && type !== 'allTypes') query.type = type;
    ...
    res.status(200).json({ data: products, totalPages: totalPages, currentPage: page });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Помилка сервера" });
  }
...
exports.getShopProducts = async (req, res) => {
  try {
    ...
    let query = {};
    if (search) query.title = { $regex: search, $options: 'i' };
    if (category && category !== 'allProducts') query.category = category;
...
exports.getUsers = async (req, res) => {
  try {
    ...
    let query = { email: { $ne: req.user.email } };
    if (search) query[searchType] = { $regex: search, $options: 'i' };
    if (role && role !== 'allRoles') query.role = role;
    ...
    res.status(200).json({ data: users, totalPages: totalPages, currentPage: page });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Помилка сервера" });
  }
...
exports.getAllOrdersForAdmin = async (req, res) => {
  try {
    ...
    if (isShippedView === true) {
      ...
      if (status && status !== 'allStatuses') query.status = status;
      else query.status = { $ne: 'Доставлено' };
    }
    ...
    let skipItems = (page - 1) * limit;
    let totalItems = await Order.countDocuments(query);
    let orders = await Order.find(query).populate('userId', 'email number address').sort(sortQuery).skip(skipItems).limit(limit);
    let totalPages = Math.ceil(totalItems / limit);

    res.status(200).json({ data: orders, totalPages: totalPages, currentPage: page });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: 'Помилка сервера' });
  }
...
exports.createOrder = async (req, res) => {
    try {
        ...
        for (let i = 0; i < cart.items.length; i++) {
            ...
            let sizeIndex = productData.sizes.findIndex(s => s.size === currentItem.size);

            if (sizeIndex === -1 || productData.sizes[sizeIndex].count < currentItem.quantity) {
                return res.status(400).json({ message: `Не вдалося оформити замовлення. Товару "${productData.title}" (Розмір: ${currentItem.size}) залишилося недостатньо.` });
            }
            ...
            productData.sizes[sizeIndex].count -= currentItem.quantity;
            await productData.save(); 
        }
...
exports.updateOrderStatusAdmin = async (req, res) => {
    try {
        const { orderId, status } = req.body;
        let updateData = { status };
        if (status === 'Доставлено') updateData.deliveryDate = new Date(); 
...
exports.createType = async (req, res) => {
    try {
        const { value, category } = req.body;
        let isType = await Type.findOne({ value });
        if (isType) return res.status(400).json({ message: "Тип з таким значенням вже існує!" });
...
exports.getTypesPaginated = async (req, res) => {
    try {
        ...
        res.status(200).json({ data: types, totalPages: totalPages, currentPage: page });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: "Помилка сервера при отриманні сторінки типів" });
    }
